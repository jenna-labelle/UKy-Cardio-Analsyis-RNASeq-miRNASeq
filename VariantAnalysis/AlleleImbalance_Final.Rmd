...
#CM variants
        ...
        merged_AllCMvcf<- lapply(merged_files, AllelicExpressionFromVCF, GOIRanges=GOIRanges, 15,2,2,3,0)
...
#NonCM variants
        ...
        #Read in vcf data, get Allelic expression stats, preliminary filtering
        merged_AllNonCMvcf<- lapply(merged_files, AllelicExpressionFromVCF, GOIRanges=NonCM_GOIRanges, 15,2,2,3,0)
...
#CM variants
        ...
        #Read in vcf data, get Allelic expression stats, preliminary filtering
        run1_AllCMvcf<- lapply(run1_files, AllelicExpressionFromVCF, GOIRanges=GOIRanges, 15,2,2,3,0)
...
#NonCM variants
        ...
        #Read in vcf data, get Allelic expression stats, preliminary filtering
        run1_AllNonCMvcf<- lapply(run1_files, AllelicExpressionFromVCF, GOIRanges=NonCM_GOIRanges, 15,2,2,3,0)
...
#CM variants
        ...
        #Read in vcf data, get Allelic expression stats, preliminary filtering
        run2_AllCMvcf<- lapply(run2_files, AllelicExpressionFromVCF, GOIRanges=GOIRanges, 15,2,2,3,0)
...
#NonCM variants
        ...
        #Read in vcf data, get Allelic expression stats, preliminary filtering
        run2_AllNonCMvcf<- lapply(run2_files, AllelicExpressionFromVCF, GOIRanges=NonCM_GOIRanges, 15,2,2,3,0)
...
wd<-"E:/UKy/Exome_Vcf/NonCMGenesROI_AllelicDepth/" #Set working directory
Samples<- c(1:3,5:19) #Samples to be read in
Files<- paste(wd, "AllelicDepth_GOIFilteredvcf_HeaderRemoved_Tube-", Samples, "-clean.snp.txt",sep="") #set file names
...
####No longer in use- for analyzing just run2 data
```{r}
...
All_sigTest$Sample<- ordered(All_sigTest$Sample, levels= c("Control",1:9,11:13,15:19))

#Split dataframe into a list of separate dfs for each gene
SplitByGene<- split(All_sigTest, f=All_sigTest$geneid) #138 genes
