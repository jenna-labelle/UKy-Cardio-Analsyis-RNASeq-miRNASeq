...
```{r}
#Read in metadata info, select just samples to be used- excluding sample 4, 10, and 14
...
```{r}
write.csv(DGERes_controlpos, paste(wd, "DESeq2/Final/", "DGERes_controlvspos_111219.csv", sep = ""))
write.csv(DGERes_controlneg, paste(wd, "DESeq2/Final/", "DGERes_controlvsneg.csv_111219", sep = ""))
write.csv(DGERes_posneg, paste(wd, "DESeq2/Final/", "DGERes_posvsneg_111219.csv", sep = ""))
```


#Plotting heatmaps: for  controls vs pos or neg analysis, for pos vs neg analysis, or for list of genes from ALL THREE analyses
```{r}
#normalize for heatmap plotting
rld<- rlog(dds)
```

#Plot heat map- for just control vs pos or neg, top 75 genes in each
```{r}
...
#plot heatmap for just the pos vs neg analysis- top 75 genes
```{r}
#settings for heatmap
subtractFromMin<- -.7
subtractFromMax<- 2.75
...
#Plot heatmap using list of genes taken from all three analysis- top 75 of control vs pos/neg and all of pos vs neg
```{r}
#settings for heatmap
subtractFromMin<- -.7
subtractFromMax<- 2.75
...
#Select only DEGs that are SPECIFIC to TTN+ or TTN-
Still working on this section

TTN+ vs TTN- analysis not considered here


#Get TTN+ and TTN- specific DEGs
```{r}
...
#select top 75 DE genes for control vs pos or neg and pos vs neg
topgenes<- c(rownames(posOnly[1:200,]), rownames(negOnly[1:1600,]))
