...
AllDepthFilteredControlJunctions<- lapply(AllSplitControlJunctions, FilterControlJunctions_BasedOnMeanDepth, Percentage=.05)
...
A_.01<-MergedAControlSpliceSites
B_.01<-MergedBControlSpliceSites

A_.05<-MergedAControlSpliceSites
B_.05<-MergedBControlSpliceSites
```
...
Filters 4 and 5:
```{r}
#Split Name into Splice junctions A and B:
...
sum(unlist(lapply(AllSplitJunctions, nrow)))
sum(unlist(lapply(AllCanonicalAnnotatedJunctions, nrow)))
...
```{r}
write.csv(multi_full, paste(wd, "AllFilteredCMJunctions_ControlRemoved10_Depth2_1CanSite_Can.05.csv"))
```
